...
//	-END : ゲーム終了
//・ゲームシーン
// - Player :WASDで動く。Enterで弾を発射する。エネミーの弾が当たるとタイトルに戻る。弾は連射できる。Enter長押しでチャージショット。
// - Enemy:上下に動く。一定間隔で弾を発射。撃つたびに弾の速度が早くなる。プレイヤーの弾が当たるとスコアが上昇する。
//　　　　　弾が当たってる間は色が変わる。弾丸を一度に複数はなてる。時間経過と共に玉が大きくなる。
